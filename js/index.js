...
function displayBookMark() {
  ...
  for (var i = 0; i < productContainer.length; i++) {
    cartona += `<tr>
            ...
            <td class=" h5"><a class="" href="${
              productContainer[i].url
            }" target="_blank">${productContainer[i].name}</a></td>
            <td><button class="btn btn-visit px-3"><i class="fa-solid fa-eye pe-2"></i><a target="_blank" href="${
              productContainer[i].url
            }">Visit</a></button></td>
...
function addBookMark() {
  if (
    ...
    bookSiteInput.value.trim() === "" ||
    !isValidURL(bookSiteInput.value)
  ) {
    var emptyInputModal = new bootstrap.Modal(
      document.getElementById("emptyInputModal")
    );
